...
def train_model(mod, n_epochs, train_loss=[], test_loss=[]):
    for i in range(n_epochs):
        ...
        with open(output_file, 'a') as f:
            # f.write(f'Train Loss Epoch {i+1}: {total_loss / trials}\n')
            f.write(f'train,{i+1},{total_loss/trials}\n')
        # print(f'Train Loss Epoch {i+1}: {total_loss / trials}\n')
        ...
        if i % 5 == 4:
            ...
            with open(output_file, 'a') as f:
                # f.write(f'Test Loss Epoch {i+1}: {total_test_loss / test_trials}\n')
